...
function resolvePersona(ctx: CommandContext, target: string): string | null {
  ...
  for (const [key, persona] of Object.entries(ctx.profile.ssh.personas)) {
...
export default defineCommand({
  ...
  complete: (ctx) => Object.values(ctx.profile.ssh.personas).map((p) => p.host),

  async run(ctx, args) {
    const target = args.raw.trim();
    const personas = ctx.profile.ssh.personas;
